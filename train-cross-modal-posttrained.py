...
from models.cross_modal import compute_cross_modal_lejepa_loss
from models.cross_modal_posttrained import CrossModalPosttrainedModel


@dataclass
class CrossModalPostTrainConfig:
    ...
    run_name: str = "CrossModalPostTrain_DESI307K_AstroCLIPPool_LeJEPA_SIGReg"
    ...
    epoch_checkpoint_every: int = 2
    keep_last_periodic_checkpoints: int = 2
    keep_last_epoch_checkpoints: int = 3
    save_dir: str = (
        "./checkpoints/CrossModalPostTrain_DESI307K_AstroCLIPPool_LeJEPA_SIGReg"
    )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="LeJEPA-align frozen pretrained image and DESI encoders"
    )
    parser.add_argument(
        "--pair-dataset-dir",
        action="append",
...
def apply_cli_overrides(config, args):
    overrides = {
        ...
        "sigreg_num_slices": args.sigreg_slices,
...
def build_sigreg(config, device):
    def make_one():
        univariate = lejepa.univariate.EppsPulley(
            n_points=config.sigreg_num_points
        )
...
def checkpoint_payload(
    ...
    return {
        "format": "cross_modal_posttrained_heads_v1",
        "alignment_heads": raw_model.alignment_state_dict(),
        "source_checkpoints": raw_model.source_metadata,
        "optimizer": optimizer.state_dict(),
        "scheduler": scheduler.state_dict(),
        "scaler": scaler.state_dict(),
        "image_sigreg": image_sigreg.state_dict(),
        "spectrum_sigreg": spectrum_sigreg.state_dict(),
        "epoch": epoch,
...
def load_checkpoint(
    ...
    if checkpoint.get("source_checkpoints") != raw_model.source_metadata:
        raise RuntimeError(
            "Resume checkpoint was created from different source backbones"
        )
    raw_model.load_alignment_state_dict(checkpoint["alignment_heads"])
    optimizer.load_state_dict(checkpoint["optimizer"])
    optimizer_to(optimizer, device)
    scheduler.load_state_dict(checkpoint["scheduler"])
    scaler.load_state_dict(checkpoint["scaler"])
    image_sigreg.load_state_dict(checkpoint["image_sigreg"])
    spectrum_sigreg.load_state_dict(checkpoint["spectrum_sigreg"])
    ...
    run = wandb.init(
        ...
        config={
            ...
            **parameter_counts,
            ...
            "training_regime": (
                "frozen_pretrained_backbones_astroclip_query_poolers"
            ),
            "objective": (
                "(1-lambda)*mean_vw||zI_v-zS_w||^2"
                "+lambda/2*(SIGReg(zI)+SIGReg(zS))"
            ),
        },
...
def main():
    config = apply_cli_overrides(CrossModalPostTrainConfig(), parse_args())
    ...
    image_sigreg, spectrum_sigreg = build_sigreg(config, device)
        ...
        while step_in_epoch < config.steps_per_epoch:
            ...
            with autocast("cuda", dtype=amp_dtype):
                ...
                losses = compute_cross_modal_lejepa_loss(
                    ...
                    image_sigreg=image_sigreg,
                    spectrum_sigreg=spectrum_sigreg,
                    lambd=config.sigreg_weight,
                )
                loss = losses["loss"]
            ...
            if should_log:
                ...
                if is_main_process(rank):
                    progress.set_postfix(
                        ...
                        cross=f"{metrics['train/cross_invariance']:.4f}",
