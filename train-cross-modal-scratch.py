...
class CrossModalTrainConfig:
    ...
    run_name: str = "CrossModalScratch_DESI307K_AllPairs_CrossOnly_SIGReg"
    wandb_mode: str = "online"
    wandb_run_id: str | None = None
    wandb_resume: str = "allow"

    log_every: int = 10
    checkpoint_every: int = 1_200
    epoch_checkpoint_every: int = 5
    keep_last_periodic_checkpoints: int = 2
    keep_last_epoch_checkpoints: int = 5
    save_dir: str = (
        "./checkpoints/CrossModalScratch_DESI307K_AllPairs_CrossOnly_SIGReg"
    )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train image and DESI encoders jointly from scratch"
    )
    parser.add_argument(
        "--pair-dataset-dir",
        action="append",
...
def main():
    config = apply_cli_overrides(CrossModalTrainConfig(), parse_args())
