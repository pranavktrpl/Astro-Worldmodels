...
@dataclass
class CrossModalScratchCLIPConfig:
    ...
    run_name: str = "CrossModalScratch_DESI307K_AllPairs_CLIP_InfoNCE"
    wandb_mode: str = "online"
    wandb_run_id: str | None = None
    wandb_resume: str = "allow"

    log_every: int = 10
    checkpoint_every: int = 2_400
    epoch_checkpoint_every: int = 5
    keep_last_periodic_checkpoints: int = 1
    keep_last_epoch_checkpoints: int = 2
    save_dir: str = (
        "./checkpoints/CrossModalScratch_DESI307K_AllPairs_CLIP_InfoNCE"
    )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train image and DESI encoders jointly from scratch with CLIP"
    )
    parser.add_argument(
        "--pair-dataset-dir",
        action="append",
    ...
    parser.add_argument("--epoch-checkpoint-every", type=int)
    parser.add_argument("--keep-last-epoch-checkpoints", type=int)
...
def apply_cli_overrides(config, args):
    overrides = {
        ...
        "epoch_checkpoint_every": args.epoch_checkpoint_every,
        "keep_last_epoch_checkpoints": args.keep_last_epoch_checkpoints,
...
def build_model(config, device, local_rank):
...
def compute_distributed_multiview_clip_loss(
    image_features,
    spectrum_features,
    logit_scale,
):
    """Average symmetric global-batch InfoNCE over all four view pairings."""
    if image_features.ndim != 3 or spectrum_features.ndim != 3:
        raise ValueError("CLIP features must have shape [views, batch, dimension]")
    ...
    if image_features.shape[0] != 2:
        raise ValueError("The matched scratch ablation requires two views")

    image = F.normalize(image_features.float(), dim=-1, eps=1.0e-3)
    spectrum = F.normalize(spectrum_features.float(), dim=-1, eps=1.0e-3)
    all_image_views = [gather_with_grad(view) for view in image]
    all_spectrum_views = [gather_with_grad(view) for view in spectrum]
    local_batch = image.shape[1]
    rank = dist.get_rank() if dist.is_initialized() else 0
    labels = rank * local_batch + torch.arange(
    ...
    spectrum_top1 = []
    positive_cosines = []
    for image_view, all_image in zip(image, all_image_views):
        for spectrum_view, all_spectrum in zip(spectrum, all_spectrum_views):
            image_logits = logit_scale * image_view @ all_spectrum.T
            spectrum_logits = logit_scale * spectrum_view @ all_image.T
            image_losses.append(F.cross_entropy(image_logits, labels))
            spectrum_losses.append(F.cross_entropy(spectrum_logits, labels))
            image_top1.append(
                (image_logits.argmax(dim=1) == labels).float().mean()
            )
            spectrum_top1.append(
                (spectrum_logits.argmax(dim=1) == labels).float().mean()
            )
            positive_cosines.append((image_view * spectrum_view).sum(-1).mean())

    image_loss = torch.stack(image_losses).mean()
    ...
    return {
        ...
        "image_to_spectrum_top1": torch.stack(image_top1).mean(),
        "spectrum_to_image_top1": torch.stack(spectrum_top1).mean(),
        "positive_cosine": torch.stack(positive_cosines).mean(),
...
def checkpoint_payload(
    ...
    return {
        ...
        "image_encoder": raw_model.image_encoder.state_dict(),
        "spectrum_encoder": raw_model.spectrum_encoder.state_dict(),
        "optimizer": optimizer.state_dict(),
        "scheduler": scheduler.state_dict(),
        "scaler": scaler.state_dict(),
        "format": "cross_modal_scratch_multiview_clip_v1",
        "objective": "mean_four_view_symmetric_global_batch_infonce",
        "epoch": epoch,
...
def load_checkpoint(
    ...
    return (
        ...
        int(checkpoint.get("step_in_epoch", 0)),
...
def setup_wandb(config, rank, dataset_info, parameter_counts):
    ...
    run = wandb.init(
        ...
        config={
            ...
            **parameter_counts,
            ...
            "objective": "mean_four_view_symmetric_global_batch_infonce",
            "negative_set": "DDP-global batch per view pairing",
            "temperature": "learned; logit scale capped at 100",
        },
...
def main():
    config = apply_cli_overrides(CrossModalScratchCLIPConfig(), parse_args())
    ...
    model = build_model(config, device, local_rank)
    ...
    if config.resume_path is not None:
        ...
        if is_main_process(rank):
            print(
                f"Resumed {config.resume_path} at epoch={start_epoch}, "
    ...
    for epoch in range(start_epoch, config.epochs):
        ...
        while step_in_epoch < config.steps_per_epoch:
            ...
            with autocast("cuda", dtype=amp_dtype):
                ...
                losses = compute_distributed_multiview_clip_loss(
                    outputs["image_projections"].float(),
                    outputs["spectrum_projections"].float(),
                    outputs["logit_scale"].float().clamp(
                        max=config.maximum_logit_scale
                    ),
                )
                loss = losses["loss"]
            ...
            if global_step % config.checkpoint_every == 0:
                ...
                if is_main_process(rank):
                    save_checkpoint(
                        ...
                        step_in_epoch,
                        global_step,
                        Path(config.save_dir) / f"step_{global_step}.pt",
                    )
                    prune_periodic_checkpoints(
                        config.save_dir,
                        config.keep_last_periodic_checkpoints,
                    )
                dist.barrier()
        ...
        if is_main_process(rank):
            ...
            should_save_epoch = (
                completed_epoch
                and (
                    next_epoch % config.epoch_checkpoint_every == 0
                    or next_epoch == config.epochs
                )
            )
            if should_save_epoch:
                save_checkpoint(
                    ...
                    next_step,
                    global_step,
                    Path(config.save_dir) / f"epoch_{next_epoch:04d}.pt",
                )
                prune_epoch_checkpoints(
                    config.save_dir,
                    config.keep_last_epoch_checkpoints,
                )
        dist.barrier()
