...
DEFAULT_CHECKPOINTS = {
    ...
    / "checkpoints/CrossModalPostTrain_DESI307K_AstroCLIPPool_LeJEPA_SIGReg/last.pt",
    "posttrained-clip-image": REPO_ROOT
    / "checkpoints/CrossModalPostTrain_DESI307K_AstroCLIP_InfoNCE/last.pt",
    "posttrained-clip-spectrum": REPO_ROOT
    / "checkpoints/CrossModalPostTrain_DESI307K_AstroCLIP_InfoNCE/last.pt",
}
...
DEFAULT_LABELS = {
    ...
    "posttrained-clip-image": "crossmodal_posttrained_clip_307k_last_image",
    "posttrained-clip-spectrum": "crossmodal_posttrained_clip_307k_last_spectrum",
}

IMAGE_ENCODERS = {"crossmodal-image", "posttrained-image", "posttrained-clip-image"}
...
def load_posttrained(
    ...
    model_classes = {
        "cross_modal_posttrained_heads_v1": CrossModalPosttrainedModel,
        "cross_modal_posttrained_astroclip_adapters_v1": CrossModalPosttrainedCLIPModel,
    }
    if checkpoint_format not in model_classes:
        raise RuntimeError(
            f"Unsupported post-training checkpoint format: {checkpoint_format}"
        )
    config = dict(checkpoint["config"])
    model = model_classes[checkpoint_format](
...
def extract(args: argparse.Namespace, checkpoint: Path, label: str) -> dict[str, Any]:
    ...
    if args.encoder in IMAGE_ENCODERS:
        ...
        if args.encoder in {"posttrained-image", "posttrained-clip-image"}:
        ...
        elif args.encoder in {"posttrained-spectrum", "posttrained-clip-spectrum"}:
            encoder, metadata = load_posttrained(checkpoint, device)
            metadata["encoder"] = args.encoder
        else:
            encoder, metadata = load_spectra_v2(checkpoint, device)
        if args.encoder in {"posttrained-spectrum", "posttrained-clip-spectrum"}:
